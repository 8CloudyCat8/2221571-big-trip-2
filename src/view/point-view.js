...
const renderModifyingPointDateTemplate = (dateFrom, dateTo, isDisabled) => (
  `<div class="event__field-group  event__field-group--time">
    ...
    &mdash;
    <label class="visually-hidden" for="event-finish-time-1">To</label>
    <input class="event__input  event__input--time" id="event-finish-time-1" type="text" name="event-finish-time" value="${getDateTime(dateTo)}" ${isDisabled ? 'disabled' : ''}>
  </div>`
);
...
const renderResetButtonTemplate = (isNewPoint, isDisabled, isDeleting) => isNewPoint ? `<button class="event__reset-btn" type="reload" ${isDisabled ? 'disabled' : ''}>Cancel</button>` : `<button class="event__reset-btn" type="reload" ${isDisabled ? 'disabled' : ''}>${isDeleting ? 'Deleting...' : 'Delete'}</button>
...
export default class PointView extends AbstractStatefulView {
  ...
  get template() {
    return createModifyingPointTemplate(this._state, this.#destinations, this.#offers, this.#isNewPoint);
  }
  ...
  #setDatepickerTo = () => {
    if (this._state.dateTo) {
      this.#datepickerTo = flatpickr(this.element.querySelector('#event-finish-time-1'), {
        enableTime: true,
        dateFormat: 'd/m/y H:i',
  ...
  #offersChangeHandler = (evt) => {
    ...
    const offers = this._state.offers.includes(offerId) ? this._state.offers.filter((offer) => offer !== offerId) : [...this._state.offers, offerId];
